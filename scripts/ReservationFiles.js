...
function sendEmailWithJQueryPostFunction()
{
    ...
    email_message = email_message + getCoronaTwintString();
...
function xmlHeaderAsString()
{
    ...
    // Header text as XML comment
    //QQQQQQ var ret_header_str = ret_header_str + g_xml_file_header_comment + newLine();
...
//    Call of getLeftTables, getMidTables and getRightTables
// 2. Get string HTML arrays of names and seats for left_tables (left_seats), mid_tables (mid_seats) 
//    and right_tables (right_seats). Calls of getColumnSeats
//    
function getNameSeatTableHtml()
{
	var ret_name_seat_html = '';
	
	var left_tables = getLeftTables();
	
	var mid_tables = getMidTables();
	
	var right_tables = getRightTables();
	
	var left_seats = getColumnSeats(left_tables);
	
	var mid_seats = getColumnSeats(mid_tables);
	
	var right_seats = getColumnSeats(right_tables);
	
	var n_number_lines = left_seats.length;
	
	if (mid_seats.length > n_number_lines)
	{
		n_number_lines = mid_seats.length;
	}
	
	if (right_tables.length > n_number_lines)
	{
		n_number_lines = right_tables.length;
	}
	
	ret_name_seat_html = ret_name_seat_html + addNameSeatStartTable();
	
	for (index_row = 0;  index_row<n_number_lines; index_row++)
	{
		ret_name_seat_html = ret_name_seat_html + addStartRow();
		
		// Left column start
		ret_name_seat_html = ret_name_seat_html + addNameSeatStartColumn();
		
		var current_left_value = "&nbsp;";
		
		if (index_row < left_seats.length)
		{
			current_left_value = left_seats[index_row];
		}
		
		ret_name_seat_html = ret_name_seat_html + current_left_value;
			
		// Left column end
		ret_name_seat_html = ret_name_seat_html + addEndColumn();
		
		// Mid column start
		ret_name_seat_html = ret_name_seat_html + addNameSeatStartColumn();
		
		var current_mid_value = "&nbsp;";
		
		if (index_row < mid_seats.length)
		{
			current_mid_value = mid_seats[index_row];
		}
		
		ret_name_seat_html = ret_name_seat_html + current_mid_value;
		...
		var current_right_value = "&nbsp;";
		
		if (index_row < right_seats.length)
		{
			current_right_value = right_seats[index_row];
		}
		
		ret_name_seat_html = ret_name_seat_html + current_right_value;
			
		// Right column end
		ret_name_seat_html = ret_name_seat_html + addEndColumn();		
		
		
		ret_name_seat_html = ret_name_seat_html + addEndRow();
		
	} // index_row
	
	
	ret_name_seat_html = ret_name_seat_html + addEndTable();
...
// 2. Loop for the table numbers
//    2.1 Get HTML array (reservation_table_array) for current table number. 
//        Call of getReservationsTable
//    2.2 Add string table and current number to the output array (index_column_output++)
//    2.3 Loop for all elements of array reservation_table_array
//        2.3.1 Add array element to output array (index_column_output++)
function getColumnSeats(i_column_tables)
{
	var ret_column_seats = [];
	
	var n_number_tables = i_column_tables.length;
	
	if (n_number_tables == 0)
	{
		alert("getColumnSeats n_number_tables = 0");
		
		return ret_column_seats;		
	}
	
	var index_column_output = 0;
	
	for (index_column_table=0; index_column_table<n_number_tables; index_column_table++)
	{
		var current_table_number = i_column_tables[index_column_table];
		
		var reservation_table_array = getReservationsTable(current_table_number);
		
		ret_column_seats[index_column_output] = "Tisch " + current_table_number;
		
		index_column_output = index_column_output + 1;
		
		for (index_column_seat=0; index_column_seat<reservation_table_array.length; index_column_seat++)
		{
			ret_column_seats[index_column_output] = reservation_table_array[index_column_seat];
...
// 2. Loop for table_number_array (and seat_char_array that has the same number of elements)
//    2.1 Get current table and seat number from arrays table_number_array and seat_char_array
//    2.2 If current table number is equal to the input table number
//        2.2.1 Get reservation name for current table and current seat. Call of getReservationName
//              For a not reserved seat the function returns "Undefined name"
//        2.2.2 Add reservation name and seat character to the output array if the seat is reserved
//              Add ------- and seat character if not
function getReservationsTable(i_table_number)
{
	...
	for (index_element=0; index_element<table_number_array.length; index_element++)
	{
		...
		if (current_table_number == i_table_number)
		{
			...
			{
				output_line = output_line + reservation_name + " " + current_seat_character;
			}
			else
			{
				output_line = output_line + "---------- " + current_seat_character;
			}
			
			ret_reservations_table[index_table_out] = output_line;
...
function getLeftTables()
{
	...
	ret_left_tables[0] = "11";
	ret_left_tables[1] = "21";
	ret_left_tables[2] = "31";
	ret_left_tables[3] = "41";

	return ret_left_tables;
	
} // getLeftTables

// Returns (column of) mid tables
// Hardcoded, but can be created using coordinates of the tables
function getMidTables()
{
	var ret_mid_tables = [];
	...
	return ret_mid_tables;
	
} // getMidTables

// Returns (column of) right tables
// Hardcoded, but can be created using coordinates of the tables
function getRightTables()
{
	var ret_right_tables = [];
	
	ret_right_tables[0] = "13";
	ret_right_tables[1] = "23";
